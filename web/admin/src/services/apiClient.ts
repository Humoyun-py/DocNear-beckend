...
if (!configuredApiBaseUrl && import.meta.env?.PROD === true) throw new Error('VITE_API_BASE_URL is required for production builds.');
const baseURL = (configuredApiBaseUrl || 'http://127.0.0.1:8001/api/v1').replace(/\/$/, '');
const ACCESS_KEY = 'docnear_admin_access', REFRESH_KEY = 'docnear_admin_refresh';
...
export function expire(){clearTokens();window.dispatchEvent(new Event('admin-auth-expired'))}
