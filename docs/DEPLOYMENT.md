...
`mobile/flutter/android/key.properties` outside Git, and use the ignored
...
Render's default Blueprint location is the repository-root `render.yaml`; it
builds the root Dockerfile, runs migrations before release, and checks
`/health/`. Configure every `sync: false` environment variable in Render before
the first deploy. Each app under `web/` has an independent `netlify.toml`; set
its Netlify base directory to that app and provide an HTTPS
`VITE_API_BASE_URL`. Netlify publishes `dist` and routes SPA requests to
`index.html`.
