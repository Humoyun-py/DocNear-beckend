...
A phone link does not create a DocNear account or issue JWTs. In the normal
flow, `/start` asks for the sender's own Telegram contact and the bot requests a
login OTP automatically, falling back to registration when no account exists.
Legacy `/code` commands remain compatibility-only. Booking ID lookup is scoped
to the linked active patient and cannot expose another user's appointment.
