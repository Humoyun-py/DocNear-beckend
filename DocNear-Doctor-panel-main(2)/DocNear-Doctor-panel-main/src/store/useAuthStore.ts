...
interface AuthState {
 ...
 restore: () => Promise<void>; requestOtp: (phone:string,channel?:'sms'|'telegram') => Promise<boolean>; verifyOtp: (credentials: OtpCredentials) => Promise<boolean>; logout: () => Promise<void>;
...
export const useAuthStore = create<AuthState>((set, get) => ({
 ...
 async requestOtp(phone,channel='sms') { set({isLoading:true,error:null}); try { await authService.requestOtp(phone,channel); set({isLoading:false}); return true; } catch(error){set({isLoading:false,error:String(error)});return false;} },
